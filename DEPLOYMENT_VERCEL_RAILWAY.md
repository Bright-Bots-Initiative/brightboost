...
3.  Copy the **Connection String** (URI). Select "Transaction Mode" (port 6543) or "Session Mode" (port 5432).
    *   *Note: For Prisma migrations (`migrate deploy`), use Session Mode (5432).*
    *   *Note: For the application connection, you can use Transaction Mode (6543) if you use a connection pooler, otherwise stick to Session Mode.*
