...
{
    /// <summary>
    /// One-click build tools for Gotcha Gears WebGL.
    /// Outputs directly to BrightBoost public folder with correct filenames.
    /// </summary>
    public static class GotchaGearsBuildTools
    {
        private const string BUILD_NAME = "gotcha_gears";
        private const string SCENE_PATH = "Assets/Scenes/GotchaGearsMain.unity";
        ...
        {
            GotchaGearsSceneBuilder.GenerateScene();
        }

        /// <summary>
        /// Build WebGL only (assumes scene already exists).
        /// Menu: Tools/BrightBoost/Gotcha Gears/Build WebGL (One Click)
        /// </summary>
        [MenuItem("Tools/BrightBoost/Gotcha Gears/Build WebGL (One Click)")]
        ...
        /// Menu: Tools/BrightBoost/Gotcha Gears/Generate + Build WebGL (One Click)
        /// </summary>
        [MenuItem("Tools/BrightBoost/Gotcha Gears/Generate + Build WebGL (One Click)")]
        ...
        /// Usage: Unity -batchmode -quit -projectPath unity-gotcha-gears -executeMethod BrightBoost.GotchaGearsBuildTools.BuildWebGL_CI
        /// </summary>
        public static void BuildWebGL_CI()
        {
            Debug.Log("[GotchaGearsBuildTools] Starting CI build...");

            // Generate scene if it doesn't exist
            if (!File.Exists(Path.Combine(Application.dataPath, "..", SCENE_PATH)))
            {
                Debug.Log("[GotchaGearsBuildTools] Scene not found, generating...");
                GotchaGearsSceneBuilder.GenerateScene();
            }
            ...
            if (!success)
            {
                Debug.LogError("[GotchaGearsBuildTools] CI build failed!");
                EditorApplication.Exit(1);
            }
            else
            {
                Debug.Log("[GotchaGearsBuildTools] CI build completed successfully!");
            }
        }

        /// <summary>
        /// Clean Build folder (preserves .gitkeep).
        /// Menu: Tools/BrightBoost/Gotcha Gears/Clean Build Folder
        /// </summary>
        [MenuItem("Tools/BrightBoost/Gotcha Gears/Clean Build Folder")]
        public static void CleanBuildFolder()
        {
            string repoRoot = Path.GetFullPath(Path.Combine(Application.dataPath, "../.."));
            string buildPath = Path.Combine(repoRoot, "public/games/gotcha-gears/Build");

            if (Directory.Exists(buildPath))
            {
                CleanBuildDirectory(buildPath);
                Debug.Log("[GotchaGearsBuildTools] Build folder cleaned (preserved .gitkeep)");
            ...
            {
                Debug.Log("[GotchaGearsBuildTools] Build folder does not exist, nothing to clean");
            }
        }
        ...
        private static bool DoBuildWebGL()
        {
            ...
            // Unity project is at <repo>/unity-gotcha-gears
            // So repo root is Application.dataPath/../..
            string repoRoot = Path.GetFullPath(Path.Combine(Application.dataPath, "../.."));
            string outputPath = Path.Combine(repoRoot, "public/games/gotcha-gears");
            string buildPath = Path.Combine(outputPath, "Build");

            Debug.Log($"[GotchaGearsBuildTools] Repo root: {repoRoot}");
            Debug.Log($"[GotchaGearsBuildTools] Output path: {outputPath}");

            // Create output directories if they don't exist
            if (!Directory.Exists(buildPath))
            {
                Directory.CreateDirectory(buildPath);
                Debug.Log($"[GotchaGearsBuildTools] Created directory: {buildPath}");
            }
            else
            {
                // Clean old build outputs (preserve .gitkeep)
                CleanBuildDirectory(buildPath);
                Debug.Log("[GotchaGearsBuildTools] Cleaned old build outputs");
            }
            ...
            try
            {
                ...
                Debug.Log("[GotchaGearsBuildTools] Starting WebGL build...");
                BuildReport report = BuildPipeline.BuildPlayer(buildOptions);
                BuildSummary summary = report.summary;

                if (summary.result == BuildResult.Succeeded)
                {
                    Debug.Log($"[GotchaGearsBuildTools] Build succeeded: {summary.totalSize} bytes");
                ...
                {
                    Debug.LogError($"[GotchaGearsBuildTools] Build failed: {summary.result}");
        ...
        /// <summary>
        /// Ensure output files have correct names (gotcha_gears.*).
        ...
        private static void EnsureCorrectFilenames(string buildPath)
        {
            ...
            foreach (string ext in extensions)
            {
                ...
                {
                    Debug.Log($"[GotchaGearsBuildTools] Found expected file: {BUILD_NAME}{ext}");
                    continue;
                }
                ...
                if (files.Length > 0)
                {
                    ...
                    Debug.Log($"[GotchaGearsBuildTools] Renaming {Path.GetFileName(sourceFile)} -> {BUILD_NAME}{ext}");
                ...
                {
                    Debug.LogWarning($"[GotchaGearsBuildTools] Missing expected file: {BUILD_NAME}{ext}");
                }
            }
        ...
        private static void CleanupExtraFiles(string buildPath)
        {
            ...
            if (File.Exists(indexHtml))
            {
                ...
                Debug.Log("[GotchaGearsBuildTools] Removed index.html");
            }
            ...
            if (Directory.Exists(templateData))
            {
                ...
                Debug.Log("[GotchaGearsBuildTools] Removed TemplateData folder");
            }
        }
        ...
        private static void CleanBuildDirectory(string buildPath)
        {
            ...
            foreach (string file in Directory.GetFiles(buildPath))
            {
                if (Path.GetFileName(file) != ".gitkeep")
                {
                    ...
                    {
                        File.Delete(file);
                        Debug.Log($"[GotchaGearsBuildTools] Deleted: {Path.GetFileName(file)}");
                    }
                    catch (System.Exception e)
                    {
                        Debug.LogWarning($"[GotchaGearsBuildTools] Failed to delete {file}: {e.Message}");
                    }
                }
            }
            ...
            foreach (string dir in Directory.GetDirectories(buildPath))
            {
                try
                {
                    ...
                    Debug.Log($"[GotchaGearsBuildTools] Deleted directory: {Path.GetFileName(dir)}");
                }
                catch (System.Exception e)
                {
                    Debug.LogWarning($"[GotchaGearsBuildTools] Failed to delete {dir}: {e.Message}");
                }
            }
            ...
            if (!File.Exists(gitkeep))
            {
                ...
                Debug.Log("[GotchaGearsBuildTools] Created .gitkeep");
            }
        }
    }
}
