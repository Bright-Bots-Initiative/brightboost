...
| Backend  | Separate project: `shared/tsconfig.json` emits CommonJS + `.d.ts` to `shared/dist/`. `backend/package.json` runs `build:shared` before `build` / `build:railway` / `typecheck`. Import via the package name `@brightboost/greatwork-engine` (`file:../shared` → `shared/package.json` `main`/`types` pointing at `dist/greatwork-engine`). **Do not** use relative paths from `backend/src/` into `shared/dist/` — with `outDir: "dist"` and `rootDir: "."`, emit lands one directory deeper (`dist/src/…`), so a relative specifier that typechecks against source depth fails at runtime (`MODULE_NOT_FOUND` under `backend/shared/dist`). |
