...
| Backend  | Separate project: `shared/tsconfig.json` emits CommonJS + `.d.ts` to `shared/dist/`. `backend/package.json` runs `build:shared` before `build:railway` / `typecheck`. Import **emitted** JS (e.g. `../../shared/dist/greatwork-engine`), not live TS outside `backend` `rootDir`. |
| Docker   | Root `Dockerfile.backend` uses `COPY . .` — that line **includes** `shared/` and must not be narrowed without proving the container still starts.                                                                                                                                 |
