...
| `scripts/verify-deploy-target.mjs`           | A deployed host is the expected commit with the right posture: staging carries `X-Robots-Tag: noindex`, production does not; `/api/health` env/sha agree; analytics not `refused`. Strict mode (`--require-declared-env`) additionally requires explicit declarations on both sides, no configuration mismatch, consistent frontend/backend environment + source + SHA, and an explicit analytics expectation (codes `DT-000`…`DT-013`) | `.github/workflows/deploy-verify.yml` and `.github/workflows/deploy-promote.yml` (`workflow_dispatch`, GitHub environment gated); promotion/rollback runbooks under `docs/brand-refresh/release-0/`; **not** in `ci-cd.yml` (post-deploy proof, not a PR gate) | `scripts/__tests__/verify-deploy-target.test.ts` — healthy strict production/staging and the compat bootstrap host pass, then each sabotage names its code (undeclared production, mismatch, FE/BE disagreement, SHA drift, unlabeled analytics, refused, missing/extra noindex); CLI exit 0 / 1 / 2 asserted | BRAND_R0 (`docs/brand-refresh/`) |
| `scripts/railway-promote.mjs`                | An exact 40-hex SHA reaches a Railway environment only when it is on `origin/main`, its required checks are green, the project is the canonical one, the environment classifies as the target, no production seed flag is set, and (production) staging is re-proven at the same SHA; both deployments must carry the SHA and pass strict verification                                                                                  | `.github/workflows/deploy-promote.yml` under the GitHub `staging` / `production` environments (the credential is an environment-scoped secret)                                                                                                                 | `scripts/__tests__/railway-promote.test.ts` — healthy inputs validate, then wrong project, short SHA, missing credential, failed/missing check, seed flags, wrong environment, FE/BE SHA drift each refuse. Live proof: a workflow run recorded in `docs/brand-refresh/release-0/evidence-register.md`        | BRAND_R0 (`docs/brand-refresh/`) |
| `scripts/staging-fixtures.mjs`               | Synthetic fixtures only ever reach the staging database: refuses the production database token, loopback, and unknown hosts; never wipes; rotates every synthetic password afterwards                                                                                                                                                                                                                                                   | Operator runbook `docs/brand-refresh/release-0/staging-runbook.md` §C (not a CI step)                                                                                                                                                                          | `scripts/__tests__/staging-fixtures.test.ts` — staging URL accepted, then production ref, unknown host, and loopback refused with exit 1; usage errors exit 2                                                                                                                                                 | BRAND_R0 (`docs/brand-refresh/`) |
