> **Canonical for:** CI jobs and parity. Last verified against code: 2026-08-10.
...
**Local–CI parity** (command table): see [`docs/agents/rules/60-verification.md`](../agents/rules/60-verification.md). Do not maintain a third copy of that table here.
...
| **build-and-test** | Lint, format, typecheck, Prisma drift, `agent:check`, `docs:check`, unit tests, then Vite boots and the **SPA shell mounts** via `npm run test:e2e:ci` (`cypress/e2e/ci-shell.cy.ts`); also produces a production build artifact | No Postgres service and no backend process. Does not hit staging or `/api` |
...
Must match issue #677. No new env vars are introduced; the per-PR shell gate needs none beyond the workflow `CYPRESS_BASE_URL` already set.
...
Run staging with `npm run test:e2e:staging` (`cypress/e2e/staging/*.cy.ts`). Also see [`docs/staging-smoke.md`](../staging-smoke.md).
