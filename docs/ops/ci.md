...
| `promote-<env>`        | Deploy promote (exact SHA) | `workflow_dispatch` only (inputs: environment, commit_sha); runs under the GitHub environment named — its required reviewer / prevent-self-review rules gate the environment-scoped `RAILWAY_TOKEN`; concurrency `deploy-promote-<env>`, `cancel-in-progress: false` | **Release-on-demand** (BRAND_R0, declared here per #775). The binding production deployment path (`scripts/railway-promote.mjs`: SHA on `main`, required checks green, canonical project, no seed flags, staging re-proof, `serviceInstanceDeployV2`, strict verification). Never a PR gate |
...
**Repository safety (#815):** `.storybook/main.ts` is a **tracked** file, and the guard used to patch it in place and restore it afterwards. It no longer does: a disposable sandbox (copy of `src/`, `shared/`, `.storybook/` and the root configs, with `public/` linked) is built before either phase, and **both phases run there** — so, as in the shell gate (#801), the only difference between them is still the patched `stories:` glob, and your checkout's files are only ever read. Restoring could not be the safety mechanism: no `finally` and no signal handler runs on `SIGKILL` (or on the hard kill Vitest/CI issue at timeout), so a hard-killed run left a modified tracked file behind.

**Why the sandbox is `.bb-guard-sandbox-*` inside the checkout, not `/tmp`:** Vite serves files only from its workspace root, which it derives from the nearest `package.json` (`searchForWorkspaceRoot`). From `/tmp` that root is the sandbox itself, so the checkout's `node_modules` is off-limits and Storybook's own setup file cannot be fetched — CI measured every story file failing to import with `Failed to fetch dynamically imported module: …/node_modules/@storybook/experimental-addon-test/dist/vitest-plugin/setup-file.mjs` and a `healthy=0` verdict. Nesting the sandbox fixes that, and it also makes path space-ness match the checkout by construction (#707). The sandbox therefore carries **no `package.json` of its own** (one would make it the workspace root again) and **no `node_modules` link** (Node already resolves up into the checkout's).

The directory is **asserted ignored by git** (`git check-ignore`) before anything is written into it — the guard refuses to run otherwise — so `git status`, the index and every tracked file are untouched, which is the invariant #815 is about. It is also ignored by ESLint and by the Vitest unit project, so a sandbox stranded by a hard kill cannot break a later command; delete it at leisure.
