...
> **Canonical for:** Biome Buddy's concept, interaction design, and prototype scope. Last verified against code: 2026-09-15.
...
| ----------- | ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Imagine** | **1 · Choose**                        | Start opens four large biome chips (K–2 direct taps), with progress dots and a `brightboost-yellow` **Select** button. Side arrows browse an unobstructed original SVG scene; its kid-level description and "who lives here" appear below the picture. The title keeps the level picker and "My Buddies" gallery.                         |
...
| 👃 **Nose & Breathing** | Gills + water-sniffing **nares** (the fish system) · Nose + lungs · Forked tongue + **Jacobson's organ** · **Spiracles** + antennae (the insect system: side ports breathe, antennae smell) | Gills only work wet — the biome dependency lesson in one part                                |
...
// nose.gills:          base { smell: 50 }   ← the nares that ride with gills
...
Kept from the draft as written: progress dots; biome scenes, descriptions and side arrows; Select/Test/Got it!/Save button placements; bars beside the Buddy with number + word band; Revise's non-intrusive advice chip. The description now sits below the biome scene so the environment remains visible.
...
- **Build a new Buddy** and the review page's **Start Biome Buddy** → `/biome-buddy?new=1`, opening Earth / Water / Fire / Air selection even when a draft exists. Use the remembered level (K–2 Guided on a new device). Browsing homes leaves the previous draft and level intact; **My Buddies** offers resume or a different level. **Select** starts a fresh recipe in the chosen home, preserving gallery entries. The entry flag remains while choosing, then is replaced out of the URL on Select or My Buddies so refresh resumes subsequent work. Direct `/biome-buddy` visits retain the existing resume behavior.
