...
| 9. Safe exploration (contract added after this doc)                                       | ✅ By construction. Variation modes per the contract: **deterministic** at the core — the engine replays tick for tick (§8.5) and the meaningful unknown is the child's own machine design; **seeded** where content does vary — generated names store a rerollable seed (§13); **remix** in Release 2, lineage reserved now (§16). The palette closure is the validated-building-blocks rule (§11.3), not runtime generation, so the generative mode does not apply. Safe way back: revise in place and Duplicate before a risky change (§11.6), no fail state in open mode (§9), and the K-2 seeded starter (§11.4). Variation never affects mastery, access, XP, or ranking (see the XP deferral below). |
