...
- Credential and relationship changes (home-access binding, login email/password, parent email) require the JWT `auth` claim to be `password` **and** re-entry of the current password; first-time home binding is proof based (owner-sent, single-use emailed token, never-bound account only). Classroom sessions (`class_code`, `class_code_pin`) and legacy tokens without the claim never gain credential authority, and no request field can widen a session. An invitation is bound to the exact enrollment that authorized it and loses authority when that enrollment, the class, its ownership or the inviter's teacher role ends — re-verified under row locks inside the accepting transaction; a fresh invitation supersedes earlier unused ones, and invitations without provenance are never claimable (**#872**).
