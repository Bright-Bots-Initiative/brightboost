...
| **build-and-test** | Lint, typecheck, unit tests, Prisma drift check, then Vite boots and the **SPA shell mounts** via `npm run test:e2e:ci` (`cypress/e2e/smoke.cy.ts`); also produces a production build artifact | No Postgres service and no backend process. Does not hit staging or `/api` |
...
| ------------------------------------------ | -------------------------------------------- | -------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------- |
| `CYPRESS_BASE_URL`                         | `build-and-test` job (workflow-level `env:`) | `http://localhost:5173`                            | **already set** — do not remove; it is what makes the shell spec work | legacy note; Cypress `baseUrl` is `CYPRESS_SWA_URL` (`smoke.cy.ts`) |
| `CYPRESS_SWA_URL`                          | per-PR shell + staging                       | `http://localhost:5173` (CI) / remapped FE locally | required by `cypress.config.ts` (`requireEnv`) — no silent fallback   | `smoke.cy.ts`, `staging/smoke.cy.ts`                                |
