...
export const parseCSVData = (csvContent: string): ParsedClassData => {
  ...
  if (lines.length < 2) {
    throw new Error(
      "CSV file must contain at least a header row and one data row. Please ensure your file has the correct format with headers like: className,grade,studentName,studentEmail,studentId",
    );
  }
  ...
  const classNameIndex = headers.findIndex(
    (h) =>
  ...
  const studentEmailIndex = headers.findIndex(
    (h) =>
  ...
  if (classNameIndex === -1) {
    throw new Error(
      `CSV must contain a class name column. Expected column names: "className", "class_name", or "class". Found columns: ${headers.join(", ")}. Please check your CSV header row.`,
    );
  }
  if (studentNameIndex === -1) {
    throw new Error(
      `CSV must contain a student name column. Expected column names: "studentName", "student_name", or "name". Found columns: ${headers.join(", ")}. Please check your CSV header row.`,
    );
  }
  ...
  dataRows.forEach((row, index) => {
    ...
    const studentEmail =
      studentEmailIndex !== -1 ? columns[studentEmailIndex] : undefined;
    const studentId =
      studentIdIndex !== -1
        ? columns[studentIdIndex]
        : `stu-${Date.now()}-${index}`;
    ...
    students.push({
      ...
      name: studentName,
      email:
        studentEmail && studentEmail.trim() !== "" ? studentEmail : undefined,
    });
  });
...
export const validateCSVData = (data: ParsedClassData): string[] => {
  ...
  if (!data.className || data.className.trim() === "") {
    errors.push(
      "Class name is required and cannot be empty. Please ensure your CSV has a valid class name.",
    );
  }

  if (data.students.length === 0) {
    errors.push(
      "At least one student is required. Please ensure your CSV contains student data with valid names.",
    );
  }
  ...
  if (duplicateNames.length > 0) {
    ...
    const actualDuplicates = uniqueDuplicates.filter(
      (name) => name !== data.className.toLowerCase().trim(),
    );
    if (actualDuplicates.length > 0) {
      errors.push(
        `Duplicate student names found: ${actualDuplicates.join(", ")}. Each student must have a unique name. Please review your CSV and ensure all student names are distinct.`,
      );
    }
  }
  ...
  if (studentEmails.length > 0) {
    ...
    if (duplicateEmails.length > 0) {
      ...
      errors.push(
        `Duplicate student emails found: ${uniqueDuplicateEmails.join(", ")}. Each student must have a unique email address. Please review your CSV and ensure all email addresses are distinct.`,
      );
    }
  }
  ...
  if (invalidEmails.length > 0) {
    errors.push(
      `Invalid email formats found: ${invalidEmails.join(", ")}. Please use valid email addresses (e.g., student@example.com).`,
    );
  }
