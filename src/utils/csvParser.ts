...
export const parseCSVData = (csvContent: string): ParsedClassData => {
  ...
  if (classNameIndex === -1) {
    throw new Error('CSV must contain a className column');
  }
  if (studentNameIndex === -1) {
    throw new Error('CSV must contain a studentName column');
  }
...
export const validateCSVData = (data: ParsedClassData): string[] => {
  ...
  if (duplicateNames.length > 0) {
    errors.push(`Duplicate student names found: ${[...new Set(duplicateNames)].join(', ')}`);
  }
  ...
  if (duplicateEmails.length > 0) {
    errors.push(`Duplicate student emails found: ${[...new Set(duplicateEmails)].join(', ')}`);
  }
