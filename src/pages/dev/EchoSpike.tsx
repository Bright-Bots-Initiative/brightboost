...
function getCtx(): AudioContext {
  if (!sharedCtx) {
    ...
    noiseBuffer = sharedCtx.createBuffer(
      1,
...
export default function EchoSpike() {
  ...
  const ensureAudio = useCallback(async (): Promise<AudioContext> => {
    ...
    if (!ready) {
      ...
      setStats((s) => ({
        ...
        output:
          typeof withOutput.outputLatency === "number"
            ? withOutput.outputLatency
            : null,
      }));
    }
    return ctx;
  ...
  const startLoop = useCallback(async () => {
    ...
    const pump = () => {
      ...
      while (
        loopStartRef.current + nextPulseRef.current * PULSE_SEC <
        now + HORIZON_SEC
      ) {
        ...
        if (pulseIdx % PULSES === 0) {
          ...
          if (scheduledCycleEventsRef.current < cycleIdx) {
            ...
            const cycleStart =
              loopStartRef.current + cycleIdx * PULSES * PULSE_SEC;
            for (const ev of eventsRef.current)
              playVoice(ev.voice, cycleStart + ev.offset);
          }
        }
      }
    };
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-4 flex flex-col items-center gap-4 touch-manipulation select-none">
      <h1 className="text-xl font-bold">
        Echo Avenue — latency spike (dev harness)
      </h1>
      ...
      <p className="text-xs text-slate-400">
        keys 1–4 work too · first tap wakes the audio engine
      </p>
      ...
      <div className="bg-slate-800 rounded-2xl p-4 w-full max-w-md font-mono text-sm flex flex-col gap-1">
        ...
        <div>
          baseLatency:{" "}
          {stats.base !== null ? `${(stats.base * 1000).toFixed(1)} ms` : "n/a"}
        </div>
        <div>
          outputLatency:{" "}
          {stats.output !== null
            ? `${(stats.output * 1000).toFixed(1)} ms`
            : "n/a (browser doesn't expose it)"}
        </div>
        <div>
          first tap (incl. engine wake):{" "}
          {stats.firstTapMs !== null
            ? `${stats.firstTapMs.toFixed(1)} ms`
            : "—"}
        </div>
        <div>
          tap→schedule delta (last {stats.deltas.length}): avg{" "}
          {avg !== null ? avg.toFixed(1) : "—"} ms · max{" "}
          {max !== null ? max.toFixed(1) : "—"} ms
        </div>
        <div className="text-emerald-300">
          est. audible latency ≈ tap-delta + output ≈{" "}
          {avg !== null && outputMs !== null
            ? (avg + outputMs).toFixed(0)
            : "—"}{" "}
          ms
          <span className="text-slate-400">
            {" "}
            (+ ~15–30 ms touch input, unmeasurable from JS)
          </span>
        </div>
      </div>
      ...
      <div className="bg-slate-800 rounded-2xl p-4 w-full max-w-md flex flex-col gap-3">
        ...
        <div className="font-mono text-sm flex flex-col gap-1">
          <div>
            cycle: {cycles} ({(cycles * CYCLE_SEC).toFixed(0)}s) · 4 pulses @
            100 BPM · snap grid ½ pulse
          </div>
          <div>
            scheduler underruns:{" "}
            <span
              className={underruns > 0 ? "text-rose-400" : "text-emerald-300"}
            >
              {underruns}
            </span>{" "}
            · worst pump gap: {worstPumpGap} ms (pump {PUMP_MS} ms, horizon{" "}
            {HORIZON_SEC * 1000} ms)
          </div>
          <div className="text-slate-400">
            drift check: events are scheduled on the AudioContext clock, so
            replay is drift-free unless underruns rise — run 60s+ and watch that
            counter.
          </div>
        </div>
      </div>
      ...
      <p className="text-xs text-slate-500 max-w-md text-center">
        Feel test: tap pads freely, then record a phrase and let it loop for a
        minute. Live = go. Rubbery = numbers below tell us which stage is eating
        the time.
      </p>
    </div>
  );
