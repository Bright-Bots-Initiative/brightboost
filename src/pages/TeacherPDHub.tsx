...
const TeacherPDHub: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "sessions" | "reflections" | "resources" | "board"
  >("sessions");
  ...
  const tabs = [
    {
      ...
      label: t("teacher.pdHub.tabSessions"),
      icon: CalendarDays,
    },
    {
      key: "reflections" as const,
      label: t("teacher.pdHub.tabReflections"),
      icon: Pencil,
    },
    {
      key: "resources" as const,
      label: t("teacher.pdHub.tabResources"),
      icon: FileText,
    },
    {
      key: "board" as const,
      label: t("teacher.pdHub.tabBoard"),
      icon: MessageSquare,
    },
  ];
    ...
    <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 text-center">
            ...
            <div className="text-xs text-gray-500">
              {t("teacher.pdHub.pdHoursCompleted")}
            </div>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 text-center">
            <CalendarDays className="w-5 h-5 text-green-600 mx-auto mb-1" />
            <div className="text-2xl font-bold text-gray-900">
              {pdHours.sessionCount}
            </div>
            <div className="text-xs text-gray-500">
              {t("teacher.pdHub.sessionsAttended")}
            </div>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 text-center">
            ...
            <div className="text-xs text-gray-500">
              {t("teacher.pdHub.reflectionsWritten")}
            </div>
          </div>
        </div>
      )}
          ...
          {showSessionForm && (
            ...
            >
              <h3 className="font-semibold text-gray-900">
                {t("teacher.pdHub.logSessionTitle")}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    {t("teacher.pdHub.dateLabel")} *
                  </label>
                  <input
                    type="date"
                    required
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={t("teacher.pdHub.topicPlaceholder")}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t("teacher.pdHub.facilitatorLabel")}
                </label>
                <input
                  type="text"
                  ...
                  placeholder={t("teacher.pdHub.facilitatorPlaceholder")}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t("teacher.pdHub.notesLabel")}
                </label>
                <textarea
                  rows={3}
                  ...
                  placeholder={t("teacher.pdHub.notesPlaceholder")}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t("teacher.pdHub.actionItemsLabel")}
                </label>
                <textarea
                  rows={3}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t("teacher.pdHub.relatedModules")}
                </label>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(MODULE_KEYS).map(([slug, key]) => (
                    <label
                      key={slug}
                      className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs cursor-pointer border transition-colors ${sessionForm.relatedModuleSlugs.includes(slug) ? "bg-blue-100 text-blue-700 border-blue-300" : "bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100"}`}
                    >
                      <input
                        type="checkbox"
                        className="sr-only"
                        ...
                        onChange={() => {
                          const updated =
                            sessionForm.relatedModuleSlugs.includes(slug)
                              ? sessionForm.relatedModuleSlugs.filter(
                                  (s) => s !== slug,
                                )
                              : [...sessionForm.relatedModuleSlugs, slug];
                          setSessionForm({
                            ...sessionForm,
                            relatedModuleSlugs: updated,
                          });
                        }}
                      />
                      {t(key)}
                ...
                >
                  ...
                  {expandedSession === session.id && (
                    <div className="mt-3 pt-3 border-t border-gray-100 space-y-2">
                      ...
                      {session.actionItems &&
                        (session.actionItems as string[]).length > 0 && (
                          <div>
                            <span className="text-xs font-medium text-gray-500">
                              {t("teacher.pdHub.actionItemsExpanded")}
                            </span>
                            <ul className="mt-1 space-y-0.5">
                              {(session.actionItems as string[]).map(
                                (item, i) => (
                      ...
                      {session.relatedModuleSlugs &&
                        (session.relatedModuleSlugs as string[]).length > 0 && (
                          <div>
                            <span className="text-xs font-medium text-gray-500">
                              {t("teacher.pdHub.relatedModulesExpanded")}
                            </span>
                            <div className="flex gap-1 mt-1">
                              {(session.relatedModuleSlugs as string[]).map(
                                (slug) => (
                                  <span
                                    key={slug}
                                    className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded"
                                  >
                                    {t(MODULE_KEYS[slug] || slug)}
                                  </span>
                                ),
                              )}
                            </div>
      ...
      {activeTab === "reflections" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900">
              {t("teacher.pdHub.tabReflections")}
            </h2>
            <button
          ...
          {showReflectionForm && (
            ...
            >
              <h3 className="font-semibold text-gray-900">
                {t("teacher.pdHub.writeReflection")}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    {t("teacher.pdHub.pdSessionLabel")} *
                  </label>
                  <select
                    required
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={t("teacher.pdHub.whatWorkedPlaceholder")}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t("teacher.pdHub.whatToChange")}
                </label>
                <textarea
                  rows={2}
                  ...
                  placeholder={t("teacher.pdHub.whatToChangePlaceholder")}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t("teacher.pdHub.studentObservations")}
                </label>
                <textarea
                  rows={2}
                  ...
                  placeholder={t(
                    "teacher.pdHub.studentObservationsPlaceholder",
                  )}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
                />
              </div>
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 transition-colors"
                >
                  {t("teacher.pdHub.saveReflection")}
                </button>
                <button
                  type="button"
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        ...
                        {r.moduleSlug &&
                          ` \u00b7 ${t(MODULE_KEYS[r.moduleSlug] || r.moduleSlug)}`}
                      </p>
                    </div>
                  </div>
                    ...
                    {r.whatWorked && (
                      <div>
                        <span className="font-medium text-green-700">
                          {t("teacher.pdHub.whatWorkedLabel")}
                        </span>{" "}
                        {r.whatWorked}
                      </div>
                    )}
                    {r.whatToChange && (
                      <div>
                        <span className="font-medium text-amber-700">
                          {t("teacher.pdHub.whatToChangeLabel")}
                        </span>{" "}
                        {r.whatToChange}
                      </div>
                    )}
                    {r.studentObservations && (
                      <div>
                        <span className="font-medium text-blue-700">
                          {t("teacher.pdHub.studentObsLabel")}
                        </span>{" "}
                        {r.studentObservations}
                      </div>
                    )}
      ...
      {activeTab === "resources" && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-900">
            {t("teacher.pdHub.pdTemplates")}
          </h2>
          <p className="text-sm text-gray-500">
            {t("teacher.pdHub.pdTemplatesDesc")}
          </p>
          {templates.length === 0 ? (
            <div className="text-center py-12 text-gray-400">
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <span className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-medium">
                      {t("teacher.pdHub.template")}
                    </span>
                  </div>
                  {tmpl.notes && (
                    <p className="text-sm text-gray-600 mt-2">{tmpl.notes}</p>
                  )}
                  {tmpl.actionItems &&
                    (tmpl.actionItems as string[]).length > 0 && (
                      <div className="mt-3">
                        <span className="text-xs font-medium text-gray-500">
                          {t("teacher.pdHub.followUpChecklist")}
                        </span>
                        <ul className="mt-1 space-y-0.5">
                          {(tmpl.actionItems as string[]).map((item, i) => (
                            <li key={i} className="text-sm text-gray-700">
                              &#9744; {item}
                            </li>
                          ))}
                  ...
                  {tmpl.relatedModuleSlugs &&
                    (tmpl.relatedModuleSlugs as string[]).length > 0 && (
                      <div className="flex gap-1 mt-3">
                        {(tmpl.relatedModuleSlugs as string[]).map((slug) => (
                          <span
                            key={slug}
                            className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded"
                          >
      ...
      {activeTab === "board" && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold text-gray-900">
            {t("teacher.pdHub.boardTitle")}
          </h2>
          <p className="text-sm text-gray-500">
            {t("teacher.pdHub.boardDesc")}
          </p>
          ...
          >
            <input
              ...
              placeholder={t("teacher.pdHub.postTitlePlaceholder")}
            ...
            <textarea
              ...
              placeholder={t("teacher.pdHub.postContentPlaceholder")}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-grow min-w-0">
                      ...
                      {post.title && (
                        <h4 className="text-sm font-medium text-gray-800 mt-0.5">
                          {post.title}
                        </h4>
                      )}
                      ...
                      {replyTo === post.id ? (
                        <div className="mt-2 flex items-center gap-2">
                          <input
                            ...
                            placeholder={t("teacher.pdHub.replyPlaceholder")}
