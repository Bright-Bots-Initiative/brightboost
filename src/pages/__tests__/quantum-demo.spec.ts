...
describe('Quantum Demo', () => {
  ...
  it('should render quantum demo page', () => {
    ...
    expect(screen.getByText('Explore quantum computing concepts through interactive gameplay!')).toBeInTheDocument();
  });
  ...
  it('should render iframe with correct attributes', () => {
    ...
    expect(iframe).toHaveAttribute('src', 'https://quantumai.google/education/thequbitgame');
  ...
  it('should grant XP on iframe load', async () => {
    ...
    const xpModule = await import('../../lib/xp');
    await waitFor(() => {
      expect(vi.mocked(xpModule.grantXp)).toHaveBeenCalledWith('quantum_demo_visit');
    });
  });
