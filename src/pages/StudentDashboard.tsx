  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const enrichedActivities = (data.activities || []).map(
          (activity: StudentActivity) => {
            const lesson = (data.enrolledLessons || []).find(
              (l: Lesson) => String(l.id) === String(activity.lessonId),
            );
            return {
              ...activity,
              lessonTitle: lesson ? lesson.title : "Activity",
            };
          },
        );
