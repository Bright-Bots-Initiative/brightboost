...
export const handler = async (
  ...
  try {
    ...
    const teacherQuery =
      "SELECT id, name, email, created_at, updated_at FROM users WHERE role = 'TEACHER' ORDER BY created_at DESC";
    const teacherResult = await db.query(teacherQuery);

    console.log(`Found ${teacherResult.rows.length} teachers`);

    return {
      statusCode: 200,
      headers,
      body: JSON.stringify(teacherResult.rows),
    };
  } catch (error) {
