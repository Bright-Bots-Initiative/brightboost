...
interface LearnerGamification {
  state: {
    ...
    // #874: lifetime best is not shared with facilitators (null); XP and
    // level are counted since the learner accepted the relationship.
...
export default function LearnerDetail() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/pathways/facilitator/learners/${userId}/gamification`, {
      headers: auth,
    })
    ...
    return (
      <div className="text-center py-20">
        <p className="text-slate-600 dark:text-slate-400">
          {t("pathways.facilitator.learnerDetail.notFound")}
        </p>
        <button
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                {data.user.name ?? t("pathways.profile.learner")}
              </h1>
              <p className="text-sm text-slate-600 dark:text-slate-400">
                {data.user.email}
              </p>
              <p className="text-xs text-slate-500 dark:text-slate-500 mt-0.5 capitalize">
                {t("pathways.facilitator.learnerDetail.band")}:{" "}
                {data.user.ageBand ?? "—"}
              </p>
            </div>
          </div>
          ...
          <CardBody>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
              ...
              <div>
                ...
                {gam.state.longestStreak !== null && (
                  <p className="text-[10px] text-slate-500">
                    best {gam.state.longestStreak}
                  </p>
                )}
              </div>
              <div>
        ...
        <CardBody>
          <ul className="space-y-2">
            {data.enrollments.map((e) => (
              <li
                key={e.id}
                className="flex items-center justify-between gap-3 p-2.5 rounded-lg border bg-slate-50 border-slate-200 dark:bg-slate-900/50 dark:border-slate-700/50"
              >
                <Link
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-600 dark:text-slate-500">
                    {t("pathways.facilitator.learnerDetail.enrolledOn", {
