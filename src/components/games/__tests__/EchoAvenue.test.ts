...
describe("unlock ladder", () => {
  const take = (lead: number, partner: number): DuetLayers => ({
    lead: Array.from({ length: lead }, (_, i) => ({
      t: i,
      soundId: "step" as const,
    })),
    partner: Array.from({ length: partner }, (_, i) => ({
      t: i + 4,
      soundId: "chime" as const,
    })),
  });
  ...
  it("FAST PACING: first phrase → Partner + 2 sounds; first layered phrase → spot + 2 more (~6 sounds in a first session)", () => {
    const p1 = advanceEchoProgress(
      FRESH_ECHO_PROGRESS,
      summarizeTake(take(2, 0)),
    );
  ...
  it("an empty take advances nothing (and costs nothing)", () => {
    const p = advanceEchoProgress(
      FRESH_ECHO_PROGRESS,
      summarizeTake(EMPTY_LAYERS),
    );
  ...
  it("newlyUnlockedSounds reports each unlock exactly once (the announce beat)", () => {
    const p1 = advanceEchoProgress(
      FRESH_ECHO_PROGRESS,
      summarizeTake(take(1, 0)),
    );
    expect(newlyUnlockedSounds("k2", FRESH_ECHO_PROGRESS, p1)).toEqual([
      "chime",
      "whoosh",
    ]);
...
describe("wondering question", () => {
  ...
  it("detects call-and-response (partner plays in the lead's gaps)", () => {
    ...
    expect(
      detectCallAndResponse({ lead: [{ t: 0, soundId: "step" }], partner: [] }),
    ).toBe(false);
  });
  ...
  it("detects rests and prioritizes: new sound > call-and-response > rests > evergreen", () => {
    ...
    expect(
      pickEchoWonder({ callAndResponse: false, restsUsed: true }).key,
    ).toBe("echoAvenue.wonder.rests");
    expect(
      pickEchoWonder({ callAndResponse: false, restsUsed: false }, () => 0.9)
        .key,
    ).toBe("echoAvenue.wonder.next");
  });
});
