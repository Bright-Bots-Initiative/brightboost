...
export function mkChallenge(
  ...
  while (mi.size < content.mysteryDrops) {
    mi.add(2 + Math.floor(Math.random() * (content.challengeRounds - 2)));
  }

  const p = pattern ? pattern.sequence : mkPattern(content).sequence;
...
function SkyShieldPlayfield({
  ...
  const T = (k: string, d: string) =>
    t(`games.skyShield.${k}`, { defaultValue: d });
  ...
  const bump = useCallback((pts: number, ok: boolean) => {
    if (ok) {
      ...
      setStreak((s) => {
        const n = s + 1;
        maxStreak.current = Math.max(maxStreak.current, n);
        return n;
      });
      correct.current++;
    } else setStreak(0);
        ...
        >
          {l === activeLane && (
            <div
              ...
              style={{ top: landed ? "calc(100% - 60px)" : "8px" }}
            >
  ...
  const HUD = () => (
    <div className="flex justify-between items-center text-sm font-bold px-2">
      <span className="bg-white/80 rounded-lg px-3 py-1 shadow-sm">
        {t("games.shared.score", { defaultValue: "Score" })}: {score}
      </span>
      {streak >= 2 && (
        <span className="streak-fire bg-orange-100 text-orange-700 rounded-lg px-3 py-1 shadow-sm">
          🔥 {streak}x
        </span>
      )}
    </div>
  ...
  const FB_MAP: Record<string, { text: string; cls: string }> = {
    ok: {
      text: T("catchGood", "Great catch!"),
      cls: "bg-emerald-100 text-emerald-700 border-emerald-300",
    },
    miss: {
      text: T("catchMiss", "Oops! Try another lane."),
      cls: "bg-red-100 text-red-700 border-red-300",
    },
    pok: {
      text: T("predictGood", "You found the pattern!"),
      cls: "bg-violet-100 text-violet-700 border-violet-300",
    },
    pmiss: {
      text: T("predictMiss", "Not quite -- keep watching!"),
      cls: "bg-orange-100 text-orange-700 border-orange-300",
    },
    sok: {
      text: T("scanGood", "Scan correct!"),
      cls: "bg-cyan-100 text-cyan-700 border-cyan-300",
    },
    smiss: {
      text: T("scanMiss", "That was a different color."),
      cls: "bg-red-100 text-red-700 border-red-300",
    },
    sfirst: {
      text: T("scanFirst", "Scan the mystery light first!"),
      cls: "bg-amber-100 text-amber-700 border-amber-300",
    },
  };
  const FBanner = () => {
    const m = fb && FB_MAP[fb];
    return m ? (
      <div
        className={`bounce-in text-center py-2 px-4 rounded-xl border font-bold text-sm ${m.cls}`}
      >
        {m.text}
      </div>
    ) : null;
    ...
    return (
      <div className="slide-up-fade text-center space-y-6 py-8">
        ...
        <h2 className="text-2xl font-extrabold text-violet-900">
          {T("title", "Sky Shield Patterns")}
        </h2>
        <p className="text-slate-600 max-w-md mx-auto">
          {T(
            "introDesc",
            "Light drops are falling! Move your shield to the right lane to catch them. Watch for patterns!",
          )}
        </p>
        <BigBtn
          onClick={() => {
            setPhase("practice");
            setPrDrop({ lane: rLane(), kind: "normal" });
            setPrReady(true);
          }}
  ...
  if (phase === "practice") {
    const doCatch = () => {
      ...
      setTimeout(() => {
        const n = prIdx + 1;
        if (n >= content.practiceRounds) {
          pat.current = mkPattern(content);
          patStarted.current = false;
          setPatShown(0);
          setPatCorrect(0);
          setPatAsking(false);
          setPhase("pattern");
    ...
    return (
      <div className="slide-up-fade space-y-4">
        ...
        <p className="text-center font-bold text-violet-800 text-sm">
          {T("practiceLabel", "Practice -- Catch the Light!")} ({prIdx + 1}/
          {content.practiceRounds})
        </p>
        <Lanes
          activeLane={prDrop.lane}
          emoji={LABELS[prDrop.lane]}
          landed={!prReady}
        />
        <FBanner />
  ...
  if (phase === "pattern") {
    ...
    if (!patStarted.current && p.length > 0) {
      ...
      const show = () => {
        setPatShown(i + 1);
        i++;
        if (i < p.length) setTimeout(show, 700);
        else setTimeout(() => setPatAsking(true), 500);
      };
      setTimeout(show, 400);
    }
    ...
    return (
      <div className="slide-up-fade space-y-4">
        <HUD />
        <p className="text-center font-bold text-violet-800 text-sm">
          {T("patternLabel", "Pattern -- Watch and Predict!")} ({patCorrect}/
          {content.patternRounds})
        </p>
        <div className="flex gap-1 justify-center flex-wrap">
          {p.slice(0, patShown).map((l, i) => (
        ...
        {patAsking && (
          <>
            <p className="text-center text-lg font-bold text-violet-900">
              {T("whichLane", "Which lane is next?")}
            </p>
        ...
        {!patAsking && (
          <p className="text-center text-sm text-slate-500">
            {T("watching", "Watching the pattern...")}
          </p>
        )}
        <FBanner />
      </div>
  ...
  // ── Phase: SCAN ─────────────────────────────────────────────────────
  if (phase === "scan") {
    const d = scDrops[scIdx];
    if (!d) return null;
    const doGuess = (c: number) => {
      if (!scRevealed) return;
      const ok = c === d.hiddenColor;
      bump(PT.scan, ok);
      showFb(ok ? "sok" : "smiss");
      setTimeout(() => {
        const n = scIdx + 1;
    ...
    return (
      <div className="slide-up-fade space-y-4">
        <HUD />
        <p className="text-center font-bold text-violet-800 text-sm">
          {T("scanLabel", "Scan -- Reveal the Mystery!")} ({scIdx + 1}/
          {scDrops.length})
        </p>
        <div className="flex justify-center">
          <span className="text-6xl bounce-in">
            {scRevealed ? LABELS[d.hiddenColor!] : "❓"}
          </span>
        </div>
        {!scRevealed && (
          <div className="text-center">
            <BigBtn
              onClick={() => setScRevealed(true)}
        ...
        {scRevealed && (
          <>
            <p className="text-center text-sm font-bold text-slate-700">
              {T("pickColor", "Pick the matching shield!")}
            </p>
            <ColorPicker onPick={doGuess} colorCount={content.mysteryColors} />
          </>
        )}
        <FBanner />
      </div>
  ...
  if (phase === "challenge") {
    ...
    const doCatch = () => {
      if (!chReady) return;
      if (isMyst && !chRevealed) {
        showFb("sfirst");
        return;
      }
      setChReady(false);
      const ok = shield === d.lane;
      bump(isMyst ? PT.scan : PT.catch, ok);
      showFb(ok ? "ok" : "miss");
      setTimeout(() => {
        const n = chIdx + 1;
        if (n >= chDrops.length) {
    ...
    return (
      <div className="slide-up-fade space-y-4">
        <HUD />
        <p className="text-center font-bold text-violet-800 text-sm">
          {T("challengeLabel", "Challenge Round!")} ({chIdx + 1}/
          {content.challengeRounds})
        </p>
  ...
  if (phase === "challengeG35") {
    ...
    const doScan = () => {
      ...
      bump(PT.predict, predictionCorrect);
    };
    ...
    return (
      <div className="slide-up-fade space-y-4">
        ...
        {/* Lanes */}
        <Lanes
          activeLane={isMystery ? (g35Scanned ? d.lane : -1) : d.lane}
          emoji={emoji}
          landed={false}
          onPick={!g35Scanned && isMystery ? doPredict : setShield}
        />
  ...
  if (phase === "exitTicket") {
    ...
    return (
      <div className="slide-up-fade space-y-6 py-4 text-center">
        <h3 className="text-xl font-extrabold text-violet-900">
          {T("exitTitle", "Exit Ticket")}
        </h3>
        <p className="text-base text-slate-700 max-w-sm mx-auto">
          {T("exitQuestion", "What comes next?")}
        </p>
        ...
        {!submitted && (
          <ColorPicker
            onPick={(c) => setExitAns(c)}
            colorCount={content.mysteryColors}
          />
        )}
        {submitted && (
            ...
            >
              {ok
                ...
                : t("games.skyShield.exitWrong", {
                    defaultValue:
                      "The answer is {{color}}. The pattern repeats!",
                    color: `${correctEmoji} ${correctColor}`,
                  })}
            </div>
            <BigBtn
  ...
  return (
    ...
    >
      {({ onFinish, reducedEffects: _reducedEffects }) => (
        <SkyShieldPlayfield band={band} onFinish={onFinish} />
      )}
    </GameShell>
  );
}
