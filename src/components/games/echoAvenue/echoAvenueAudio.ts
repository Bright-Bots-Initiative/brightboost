...
export function getEchoAudio(): EchoAudio {
  ...
  const out = (performer: "lead" | "partner") =>
    performer === "lead" ? lead : partner;
  ...
  function osc(
    ...
    if (f1 !== null)
      o.frequency.exponentialRampToValueAtTime(f1, when + dur * 0.9);
  ...
  function noiseHit(
    ...
    if (freq1 !== null)
      bp.frequency.exponentialRampToValueAtTime(freq1, when + dur * 0.9);
    bp.Q.value = q;
    const g = ctx.createGain();
    g.gain.setValueAtTime(peak, when);
    g.gain.exponentialRampToValueAtTime(0.001, when + dur);
