 ...
 */
export function quantizeToSubdivision(
  rawOffsetSec: number,
  cfg: BandConfig,
): number {
...
export function advanceEchoProgress(
  p: EchoProgress,
  take: TakeSummary,
): EchoProgress {
...
export function detectRests(layers: DuetLayers, cfg: BandConfig): boolean {
  ...
  const occupied = new Set(
    [...layers.lead, ...layers.partner].map((ev) => ev.t),
  );
...
export function pickEchoWonder(
  ...
  if (ctx.newSoundUsed)
    return { key: `echoAvenue.wonder.newSound.${ctx.newSoundUsed}` };
...
export const ECHO_TOKENS_FIRST: NameToken[] = [
  {
    id: "midnight",
    label: { en: "Midnight", es: "Medianoche", "zh-CN": "午夜" },
  },
  ...
  {
    id: "electric",
    label: { en: "Electric", es: "Eléctrico", "zh-CN": "闪电" },
  },
];
...
export function buildDuetContent(
  ...
  return {
    ...
    name,
    band,
    pulses: BAND_CONFIG[band].pulses,
    layers,
    spots,
    coverPose,
  };
