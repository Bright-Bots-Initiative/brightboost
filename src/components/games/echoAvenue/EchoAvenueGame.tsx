...
function PartnerFigure({ motion }: { motion: string | null }) {
  ...
  return (
    ...
    >
      ...
      <polyline
        points="24,26 34,32 44,26 54,32"
        stroke="#facc15"
        strokeWidth="5"
        fill="none"
        strokeLinecap="round"
...
export function EchoStudioCore({
  ...
  const [lightFlash, setLightFlash] = useState<{
    color: string;
    key: number;
  } | null>(null);
  const [trails, setTrails] = useState<
    { id: number; performer: Performer; icon: string }[]
  >([]);
  ...
  const motionTimers = useRef<
    Record<Performer, ReturnType<typeof setTimeout> | null>
  >({
    lead: null,
    partner: null,
  });
  ...
  const triggerMotion = useCallback(
    (who: Performer, soundId: SoundId) => {
      ...
      if (!calm) {
        ...
        setTrails((prev) => [
          ...prev.slice(-7),
          { id, performer: who, icon: spec.icon },
        ]);
      }
    },
    [calm],
            ...
            for (const who of ["lead", "partner"] as Performer[]) {
              for (const ev of layersRef.current[who]) {
                audio.play(
                  ev.soundId,
                  who,
                  eventTime(loopStartRef.current, cycleIdx, ev.t, cfg),
                );
              }
            }
          }
  ...
  useEffect(() => {
    ...
    const frame = () => {
      ...
      if (audio) {
        const tIn =
          (audio.now() - loopStartRef.current + cycleSec(cfg)) % cycleSec(cfg);
    ...
    (m: Mood) => {
      ...
      for (const ev of seed) {
        audio.play(
          ev.soundId,
          "lead",
          start + (ev.t * cfg.pulseSec) / cfg.snapPerPulse,
        );
      }
    },
  ...
  const stopTake = useCallback(() => {
    ...
    const queue: string[] = [];
    if (!partnerUnlocked(band, before) && partnerUnlocked(band, after))
      queue.push("partner");
    for (const s of newlyUnlockedSounds(band, before, after))
      queue.push(`sound.${s}`);
    if (!spotsUnlocked(band, before) && spotsUnlocked(band, after))
      queue.push("spots");
    if (queue.length > 0) {
  ...
  const saveDuet = useCallback(async () => {
    ...
    try {
      if (latestRef.current.creationId) {
        await api.patch(`/creations/${latestRef.current.creationId}`, {
          content,
        });
      } else {
  ...
  const done = useCallback(() => {
    ...
    const familiesExplored = new Set(
      [
        ...latestRef.current.layers.lead,
        ...latestRef.current.layers.partner,
      ].map((ev) => soundSpec(ev.soundId).family),
    ).size;
    onFinish(
      buildEchoAvenueResult({
      ...
      >
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {!watchOnly &&
            (["together", "echo", "space"] as Mood[]).map((m) => (
              ...
              >
                {m === "together"
                  ? t("echoAvenue.title.moodTogether", {
                      defaultValue: "🤝 Together",
                    })
                  : m === "echo"
                    ? t("echoAvenue.title.moodEcho", {
                        defaultValue: "🔁 Echo",
                      })
                    : t("echoAvenue.title.moodSpace", {
                        defaultValue: "🌙 Space",
                      })}
              </button>
            ))}
        ...
        {draft && (
          <p className="text-sm text-slate-500 font-semibold">
            {t("echoAvenue.title.resume", {
              defaultValue: "Your last take is waiting inside.",
            })}
          </p>
        )}
      </div>
    ...
    <div className="flex flex-col items-center gap-3 w-full px-2 pb-4">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-xs font-extrabold text-indigo-900">
              {hasPartner
                ...
                : t("echoAvenue.studio.partnerLocked", {
                    defaultValue: "Partner (record a take!)",
                  })}
            </span>
          </div>
        </div>
      ...
      {watching ? (
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {watchOnly ? (
            ...
            >
              {t("echoAvenue.watch.doneWatching", {
                defaultValue: "Done watching",
              })}
            </button>
          ) : (
            <>
              ...
              <button
                ...
                className="min-h-12 px-8 rounded-full bg-white border-2 border-slate-300 font-extrabold text-slate-700 active:scale-95 touch-manipulation focus-visible:outline focus-visible:outline-2 focus-visible:outline-sky-500"
              >
                {t("echoAvenue.watch.changeLayer", {
                  defaultValue: "🔁 Change a layer",
                })}
              </button>
            </>
          )}
        ...
        <>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 w-full max-w-xl">
            ...
            <label className="flex items-center gap-1 text-xs font-bold text-slate-600">
              {t("echoAvenue.studio.layerVolume", {
              ...
              <input
                ...
                defaultValue={1}
                onChange={(e) =>
                  audioRef.current?.setLayerVolume(
                    performer,
            ...
            <button
              ...
              aria-label={t("echoAvenue.sensory.muteAria", {
                defaultValue: "Toggle sound",
              })}
            >
              {muted ? "🔇" : "🔊"}
            </button>
            ...
            {pads.map((soundId, i) => {
              ...
              return (
                <button
                  ...
                  aria-label={t(`echoAvenue.sound.${soundId}`, {
          ...
          <div className="flex flex-wrap justify-center gap-2">
            ...
            >
              {recording
                ? t("echoAvenue.studio.stopTake", {
                    defaultValue: "⏹ Finish the take",
                  })
                : t("echoAvenue.studio.record", {
                    defaultValue: "● Record a take",
                  })}
            </button>
            <button
              type="button"
            ...
            >
              {t("echoAvenue.studio.watch", {
                defaultValue: "🎬 Watch the Take",
              })}
            </button>
            <button
              type="button"
              ...
              className="min-h-11 px-4 rounded-full bg-white border-2 border-slate-300 font-bold text-slate-700 active:scale-95 touch-manipulation focus-visible:outline focus-visible:outline-2 focus-visible:outline-sky-500"
            >
              {t("echoAvenue.studio.redoLayer", {
                defaultValue: "Redo this layer",
              })}
            </button>
            <button
              type="button"
          ...
          <p className="text-xs text-slate-500 font-semibold" role="status">
            {name
              ? `🏷️ ${name}`
              : t("echoAvenue.studio.untitled", {
                  defaultValue: "Untitled take",
                })}
            {saveNote === "saved" &&
              ` · ${t("echoAvenue.studio.saved", { defaultValue: "Saved ✓" })}`}
            {saveNote === "local" &&
          ...
          <h3 className="text-xl font-extrabold text-slate-800" role="status">
            {announceQueue[0] === "partner"
              ? t("echoAvenue.unlock.partner", {
                  defaultValue:
                    "Your Partner joins the stage! Overdub a second layer!",
                })
              : announceQueue[0] === "spots"
                ? t("echoAvenue.unlock.spots", {
                    defaultValue:
                      "Sound spots opened! The tunnel and the puddle color your sounds!",
                  })
                : t(
                    `echoAvenue.unlock.${announceQueue[0].replace("sound.", "")}`,
                    {
      ...
      {wonder && (
        <Overlay>
          ...
          >
            {t("echoAvenue.reflect.backToStudio", {
              defaultValue: "Back to my studio",
            })}
          </button>
        </Overlay>
      )}
      ...
      {showNaming && (
        <Overlay>
          ...
          <p className="text-sm font-bold text-slate-600">
            {t("echoAvenue.name.poseTitle", {
              defaultValue: "Pick a cover pose",
            })}
          </p>
          <div className="flex justify-center gap-2">
            {COVER_POSES.map((pose) => (
              <button
                ...
                aria-label={t(`echoAvenue.pose.${pose}`, {
                  defaultValue: pose,
                })}
...
export default function EchoAvenueGame({
  ...
  const { t } = useTranslation();
  const band: EchoBand =
    (config as { gradeBand?: string } | undefined)?.gradeBand === "g3_5"
      ? "g35"
      : "k2";
  ...
  const fixedDuet =
    rawDuet &&
    typeof rawDuet === "object" &&
    Array.isArray((rawDuet as SoundDuetContent).layers?.lead)
