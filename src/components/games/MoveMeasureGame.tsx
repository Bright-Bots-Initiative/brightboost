...
export function tossScore(v: number) {
  return Math.max(0, Math.round(10 - Math.abs(v - IDEAL_TOSS) * 0.2));
}
...
function ZoneBar({
  ...
  return (
    <div
      className={
        vertical
          ? "relative w-16 h-64 rounded-2xl bg-slate-200 overflow-hidden mx-auto"
          : "relative w-full h-16 rounded-2xl bg-slate-200 overflow-hidden"
      }
    >
      <div className="absolute bg-emerald-300/60 rounded" style={green} />
      <div
        className={`absolute w-6 h-6 rounded-full shadow-lg border-2 transition-colors ${stopped ? (inZone ? "bg-emerald-500 border-emerald-700" : "bg-red-400 border-red-600") : "bg-amber-400 border-amber-600"}`}
        style={marker}
      />
    </div>
  );
...
function ScoreFeedback({
  ...
  const label =
    score >= 8
      ? t("games.moveMeasure.great", { defaultValue: "Great!" })
      : score >= 5
        ? t("games.moveMeasure.good", { defaultValue: "Good!" })
        : t("games.moveMeasure.tryHarder", { defaultValue: "Keep trying!" });
  return (
...
function MoveMeasurePlayfield({
  ...
  // ── Dash anim ──
  useEffect(() => {
    if ((phase !== "dash" && !(isRetry && impEvent === "dash")) || dashDone)
      return;
    let p = 0;
    const go = () => {
      p += 0.012;
  ...
  useEffect(() => {
    if (
      (phase !== "jump" && !(isRetry && impEvent === "jump")) ||
      !jHold ||
      jDone
    )
      return;
  ...
  const clearWinner: EventKey | null =
    bestEvents.length === 1 ? bestEvents[0] : null;
  ...
  const tips = [
    {
      label: t("games.moveMeasure.tipFocus", {
        defaultValue: "Focus on timing",
      }),
      ev: "dash" as EventKey,
    },
    {
      label: t("games.moveMeasure.tipPower", {
        defaultValue: "Control your power",
      }),
      ev: "jump" as EventKey,
    },
    {
      label: t("games.moveMeasure.tipAim", { defaultValue: "Aim carefully" }),
      ev: "toss" as EventKey,
    },
  ];
  ...
  const exits = [
    {
      text: t("games.moveMeasure.exitA", { defaultValue: "I guessed" }),
      ok: false,
    },
    {
      text: t("games.moveMeasure.exitB", {
        defaultValue: "I measured and compared",
      }),
      ok: true,
    },
    {
      text: t("games.moveMeasure.exitC", { defaultValue: "I asked a friend" }),
      ok: false,
    },
  ];
  ...
  const evHeader = (
    ...
    <>
      <p className="text-sm font-bold uppercase tracking-widest text-emerald-600">
        {isRetry
          ? t("games.moveMeasure.retryLabel", { defaultValue: "Retry!" })
          : eventNum}
      </p>
      <h3 className="text-2xl font-extrabold text-slate-800">
        {icon} {t(titleKey, { defaultValue: dv })}
      </h3>
      <p className="text-base text-slate-600">
        {t(instrKey, { defaultValue: instrDv })}
      </p>
    </>
  );
  ...
  if (phase === "predict") {
    ...
    const value = predictions[currentEvent] ?? range.min;
      ...
      <div className="slide-up-fade text-center space-y-6 py-6">
        ...
        <p className="text-base text-slate-600">
          {t("games.moveMeasure.predictText", {
            defaultValue:
              "Before you try, predict where you think you'll land!",
          })}
        </p>
  ...
  if (phase === "dash" || (isRetry && impEvent === "dash")) {
    ...
    return (
      <div className="slide-up-fade text-center space-y-6 py-6">
        ...
        {!dashDone && (
          <BigBtn gradient="from-amber-400 to-amber-500" onClick={tapDash}>
            {t("games.moveMeasure.tap", { defaultValue: "TAP!" })}
          </BigBtn>
        )}
        {sc !== null && (
          <>
            ...
            {config.showScore && <ScoreFeedback score={sc} t={t} />}
          </>
        )}
      </div>
    );
      ...
      <div className="slide-up-fade text-center space-y-6 py-6">
        ...
        <div className="max-w-sm mx-auto space-y-4">
          <input
            ...
            max={100}
            value={tVal}
            onChange={(e) => !tDone && setTVal(Number(e.target.value))}
            disabled={tDone}
            className="w-full h-3 rounded-lg appearance-none cursor-pointer accent-emerald-500"
          />
          <div className="flex justify-between text-xs text-slate-400 font-bold">
        ...
        {!tDone && (
          <BigBtn gradient="from-purple-400 to-purple-500" onClick={relToss}>
            {t("games.moveMeasure.throw", { defaultValue: "THROW!" })}
          </BigBtn>
        )}
        {sc !== null && (
          <>
  ...
  if (phase === "compare") {
    ...
    return (
      <div className="slide-up-fade text-center space-y-6 py-6">
        <h3 className="text-2xl font-extrabold text-slate-800">
          📊{" "}
          {t("games.moveMeasure.compareTitle", {
            defaultValue: "Compare Your Results",
          })}
        </h3>

        {allPerfect && (
          <p className="bounce-in text-xl font-extrabold text-emerald-700">
            {t("games.moveMeasure.acedEvery", {
              defaultValue: "You aced every event! 🏆",
            })}
          </p>
        )}
          ...
          {(["dash", "jump", "toss"] as const).map((ev) => {
            ...
            return (
              <div key={ev} className="flex flex-col items-center gap-2">
                ...
                <div
                  className="w-12 bg-slate-200 rounded-full overflow-hidden"
                  style={{ height: 120 }}
                >
                  <div
        ...
        <div className="flex justify-center gap-3 flex-wrap">
          {(["dash", "jump", "toss"] as const).map((ev) => {
            ...
            const isCorrect = preferenceMode
              ? isPicked
              : isPicked && ev === clearWinner;
  ...
  if (phase === "improve") {
    return (
      <div className="slide-up-fade text-center space-y-6 py-6">
        <h3 className="text-2xl font-extrabold text-slate-800">
          🔬{" "}
          {t("games.moveMeasure.improveTitle", {
            defaultValue: "Pick a Coaching Tip",
          })}
        </h3>
  ...
  return (
    ...
    >
      {({ onFinish, reducedEffects: _reducedEffects }) => (
        <MoveMeasurePlayfield band={band} onFinish={onFinish} />
      )}
    </GameShell>
  );
}
