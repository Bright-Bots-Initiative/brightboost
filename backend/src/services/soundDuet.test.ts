...
describe("validateSoundDuet — schema", () => {
  ...
  it("rejects missing/empty/oversized names", () => {
    ...
    expect(validateSoundDuet({ ...valid(), name: "x".repeat(25) }).ok).toBe(
      false,
    );
  });
...
describe("validateSoundDuet — validity guard", () => {
  ...
  it("rejects layers over the event cap (64)", () => {
    const c = valid();
    c.layers.lead = Array.from({ length: 65 }, () => ({
      t: 0,
  ...
  it("rejects an unknown cover pose", () => {
    expect(validateSoundDuet({ ...valid(), coverPose: "tPose" }).ok).toBe(
      false,
    );
    expect(validateSoundDuet({ ...valid(), coverPose: undefined }).ok).toBe(
      false,
    );
  });
  ...
  it("rejects spot lists over the two-spot cap and unknown spots", () => {
    expect(
      validateSoundDuet({ ...valid(), spots: ["tunnel", "puddle", "tunnel"] })
        .ok,
    ).toBe(false);
...
describe("payload-size cap (defense-in-depth)", () => {
  it("a worst-case VALID duet stays under the cap", () => {
    const maxed = {
      ...
      name: "x".repeat(24),
      layers: {
        lead: Array.from({ length: 64 }, (_, i) => ({
          t: i % 16,
          soundId: "twinkle" as const,
        })),
        partner: Array.from({ length: 64 }, (_, i) => ({
          t: i % 16,
          soundId: "twinkle" as const,
        })),
      },
  ...
  it("rejects oversized and unserializable payloads before parsing", () => {
    expect(
      exceedsDuetPayloadCap({ blob: "y".repeat(MAX_DUET_PAYLOAD_CHARS + 1) }),
    ).toBe(true);
    const oversized = validateSoundDuet({
      blob: "y".repeat(MAX_DUET_PAYLOAD_CHARS + 1),
    });
