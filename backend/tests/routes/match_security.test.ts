...
describe('Match Routes - Queue', () => {
  ...
  it('should accept valid bands', async () => {
    ...
    // Mock create match
    (prisma.match.create as any).mockResolvedValue({ id: 'match-new', status: 'PENDING' });
  ...
  it('should prevent multiple pending matches for the same user', async () => {
    ...
    (prisma.match.findFirst as any).mockResolvedValueOnce({ id: 'match-existing', status: 'PENDING' });
