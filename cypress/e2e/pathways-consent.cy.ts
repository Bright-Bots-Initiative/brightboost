...
describe("#874 Pathways consent confirmation (live stack)", () => {
  ...
  it("R1: a revoked learner cannot restore access with the code — in English and Spanish", () => {
    ...
    loginApi(contract.pathways.learners.revoked).then((rev) => {
  ...
  it("K1: keyboard operation, an invalid code, and preview + cancel write nothing", () => {
    ...
    loginApi(contract.pathways.learners.trusted).then((learner) => {
      ...
      api(
        ...
        expect(p.status).to.eq(200);
        expect(p.body.enrollment.state, "preview + cancel wrote nothing").to.eq(
          "none",
        );
      });
      api(learner, "GET", "/api/pathways/student/home").then((h) => {
        const names = (h.body.enrollments as Array<{ cohortName: string }>).map(
          (e) => e.cohortName,
        );
  ...
  it("L1: a legacy learner signs in through the UI and confirms; visibility resumes with prior history protected", () => {
    ...
    loginApi(contract.pathways.facilitatorEmail).then((fac) => {
      cohortIdByName(fac, primary.name).then((cohortId) => {
        ...
        cy.window().then((win) => {
          ...
          api(fac, "GET", `/api/pathways/facilitator/learners/${user.id}`).then(
            (d) => expect(d.status, "hidden before consent").to.eq(404),
          );
  ...
  it("T1: a track change between preview and confirmation grants nothing unseen and requires renewed consent", () => {
    ...
    loginApi(contract.pathways.facilitatorEmail).then((fac) => {
      cohortIdByName(fac, second.name).then((secondId) => {
        loginApi(contract.pathways.learners.legacy).then((learner) => {
          visitAs(learner, "/pathways/join");
      ...
      cohortIdByName(fac, primary.name).then((cohortId) => {
        ...
        loginApi(contract.pathways.learners.trusted).then((learner) => {
          ...
          cy.get(`[data-testid="consent-prompt-${cohortId}"]`, {
            ...
            .within(() => cy.contains("button", "Review and confirm").click());
          ...
          api(
            ...
            expect(earlier.consentedSince, "earlier boundary unchanged").to.eq(
  ...
  it("E1: success and error states render in Spanish", () => {
    ...
    loginApi(contract.pathways.facilitatorEmail).then((fac) => {
      cohortIdByName(fac, primary.name).then((cohortId) => {
        loginApi(contract.pathways.learners.legacy).then((learner) => {
          // A1 added a track to the primary cohort; this learner has not
          // confirmed it yet.
          ...
          cy.get(`[data-testid="consent-prompt-${cohortId}"]`, {
            ...
            .within(() => cy.contains("button", "Revisar y confirmar").click());
          cy.get('[data-testid="join-state"]').should(
            "contain",
